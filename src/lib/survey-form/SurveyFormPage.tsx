...
const BASE_QUESTION_KEYS = [
	...
	'electricitySourceId',
	'entranceAccessId',
	'gardenEntranceAccessId',
	'groundSurfaceConditionId',
	'landPreparations',
	'soilMoistureId',
	'fixedStructures',
	'surveyorNote',
...
function isFilled(data: SurveyFormData, key: string) {
	switch (key) {
		case 'areaSunExposureTimes':
			return (data.areaSunExposureTimes?.length ?? 0) > 0 && !!data.sunExposureObstructionId;

		case 'waterSourceId':
			return !!data.waterSourceId && !!data.electricitySourceId;

		case 'entranceAccessId':
			return !!data.entranceAccessId && !!data.gardenEntranceAccessId;

		case 'landPreparations':
			return (data.landPreparations?.length ?? 0) > 0;

		case 'fixedStructures':
			return (data.fixedStructures?.length ?? 0) > 0;
		...
		case 'areaPhoto':
			return (
				...
				!!data.areaPhotoCloudStorageUrl
			);

		case 'plantPresence':
			return (
				(data.plantPresences?.length ?? 0) > 0 ||
...
function calcSurveyProgress(
	data: SurveyFormData,
	extraFields: Stage2FieldKey[],
	isOngoing: boolean
) {
	const keys = [...BASE_QUESTION_KEYS];

	if (isOngoing) {
		// Tahap 2 hanya menghitung field yang benar-benar ditambahkan
		keys.splice(
			13,
			6,
			...extraFields
	...
	const filled = keys.reduce(
		(sum, key) => sum + (isFilled(data, key) ? 1 : 0),
		0
	);
...
function isEntryEmpty(entry: SurveyEntry) {
	const stage1Keys = BASE_QUESTION_KEYS.slice(0, 13);  // gardenName ... surveyorNote
	const stage2Keys = BASE_QUESTION_KEYS.slice(13, 19);  // areaSunExposureId ... gardenThemeId

	const hasStage1 = stage1Keys.some((key) => isFilled(entry.data, key));
	const hasStage2 = stage2Keys.some((key) => isFilled(entry.data, key));

	return !hasStage1 && !hasStage2 && entry.extraFields.length === 0;
}
	...
	async function performSubmitAll() {
		...
		try {
			...
			if (allStage2Complete) {
				...
				addToast({
					data: {
						title: 'Survey berhasil dikirim & menunggu peninjauan',
						color: '#4EA40F',
						bg: '#EAFACC',
						border: '#4EA40F',
						icon: 'check_box'
				...
				const progressList = updatedEntries.map((entry) =>
					calcSurveyProgress(
						entry.data,
						entry.extraFields,
				...
				const totalFilled = progressList.reduce(
					(sum, p) => sum + p.filled,
					0
				);
...
const progress = calcSurveyProgress(
	activeEntry.data,
	activeExtraFieldKeys,
	isOngoing
);
const activeProgress = progress.filled;
const activeTotalSections = progress.total;
			...
			{isStage2OrLengkapi ? (
				<div className="px-4 py-2 border-b border-neutral-100">
					<p className="text-xs text-neutral-500">
						{progress.filled} dari {progress.total} Pertanyaan
					</p>
				</div>
			) : (
		...
		<main ref={containerRef} className="pb-17 bg-neutral-50 min-h-screen">
			...
			{isOngoing ? (
				...
				<>
					...
					{/* Progress bar + info stage */}
					<div className="mx-3 mb-2 bg-white rounded-md border border-neutral-8 overflow-hidden">
						{stageProgressInfo}
					</div>
				</>
			)}
